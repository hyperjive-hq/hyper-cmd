...
class MCPConfigGenerator:
    ...
    def generate_config(cls) -> dict[str, Any]:
        ...
        return {
            ...
            "version": "1.0",
            "description": "MCP configuration for Hyper CLI integration - auto-generated by hyper init-mcp",
        }
    ...
    def merge_config(
        ...
        # Add or update hyper-core server
...
class McpInitCommand(BaseCommand):
    ...
    def help_text(self) -> str:
        ...
        - Preserves other MCP servers while adding/updating hyper-core
    ...
    def _determine_merge_strategy(self, config_file: Path, force: bool) -> Optional[str]:
        ...
        try:
            ...
            # Check if hyper-core already exists
            has_hyper_core = "hyper-core" in existing_servers
            has_other_servers = len([k for k in existing_servers.keys() if k != "hyper-core"]) > 0
            ...
            if has_other_servers:
                ...
                if has_hyper_core:
                    self.print_warning("hyper-core server already exists in configuration")

                return self._prompt_merge_strategy(has_hyper_core)
            elif has_hyper_core:
                self.print_warning(f"hyper-core server already exists at {config_file}")
    ...
    def _prompt_merge_strategy(self, has_hyper_core: bool) -> Optional[str]:
        """Prompt user to choose merge strategy when other servers exist."""
        try:
            if has_hyper_core:
                self.console.print("\nOptions:")
                self.console.print("  [1] Merge - Update hyper-core server, keep other servers")
                self.console.print("  [2] Overwrite - Replace entire file with only hyper-core")
                ...
                self.console.print("\nOptions:")
                self.console.print("  [1] Merge - Add hyper-core server, keep existing servers")
                self.console.print("  [2] Overwrite - Replace entire file with only hyper-core")
                self.console.print("  [3] Cancel")
    ...
    def _show_config_preview(self, config: dict[str, Any], config_file: Path) -> None:
        ...
        self.console.print("  📋 Server name: [cyan]hyper-core[/cyan]")
    ...
    def _show_merge_preview(
        ...
        kept_servers = [name for name in existing_servers.keys() if name != "hyper-core"]
        if kept_servers:
            self.console.print("  ✓ Keeping existing servers:")
            for server_name in kept_servers:
                self.console.print(f"    • [cyan]{server_name}[/cyan]")

        # Show hyper-core server (new or updated)
        if "hyper-core" in existing_servers:
            self.console.print("  🔄 Updating server:")
        else:
            self.console.print("  ➕ Adding new server:")

        hyper_config = merged_servers["hyper-core"]
        self.console.print(f"    • [green]hyper-core[/green] - {hyper_config['description']}")
