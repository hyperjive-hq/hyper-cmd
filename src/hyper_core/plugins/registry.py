...
class PluginRegistry:
    ...
    def initialize(
        self,
        plugin_paths: Optional[list[Union[str, Path]]] = None,
        ...
        if config.has_hyper_directory():
            ...
            default_paths = [Path.cwd() / "plugins", Path.home() / ".hyper" / "plugins"]
    ...
    def _discover_and_register_components(self, metadata: PluginMetadata):
        ...
        for _name, obj in inspect.getmembers(metadata.module):
            if inspect.isclass(obj):
                # Check if it's a command
                if self._is_command(obj) and obj.__module__.startswith(
                    f"hyper_plugins.{plugin_name}"
                ):
                    self.register_command(obj, plugin_name)

                # Check if it's a widget
                elif self._is_widget(obj) and obj.__module__.startswith(
                    f"hyper_plugins.{plugin_name}"
                ):
                    self.register_widget(obj, plugin_name)

                # Check if it's a page
                elif self._is_page(obj) and obj.__module__.startswith(
                    f"hyper_plugins.{plugin_name}"
                ):
                    self.register_page(obj, plugin_name)

                # Check if it's a service
                elif self._is_service(obj) and obj.__module__.startswith(
                    f"hyper_plugins.{plugin_name}"
                ):
    ...
    def _get_component_name(self, component_class: type, component_type: str) -> str:
        ...
        if hasattr(component_class, "name"):
            try:
                ...
                name_attr = component_class.name
