...
class PluginDiscovery:
    ...
    def _is_potential_plugin(self, path: Path) -> bool:
        ...
        return (
            path.is_dir() and 
            not path.name.startswith('.') and  # Skip hidden directories
            not path.name.startswith('_')      # Skip private directories
        )
    ...
    def _is_valid_plugin(path: Path) -> bool:
        ...
        """
        return (
            (path / "__init__.py").exists() and  # Must be a Python package
            (path / "plugin.py").exists()        # Must have main plugin module
        )
...
class PluginLoader:
    ...
    def _extract_plugin_info(self, module: Any, manifest: Optional[Dict[str, Any]]) -> Dict[str, Any]:
