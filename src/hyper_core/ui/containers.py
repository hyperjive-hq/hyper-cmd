...
from .engine import UIComponent, RenderContext, RenderState
...
class BorderedContainer(UIComponent):
    ...
    def _draw_border(self, ctx: RenderContext) -> None:
        ...
        ctx.window.add_ch(ctx.y + ctx.height - 1, ctx.x + ctx.width - 1, BoxChars.ACS_LRCORNER or BoxChars.LRCORNER)
...
class FlexContainer(UIComponent):
    ...
    def add_child_with_config(self, child: UIComponent, 
    ...
    def _calculate_allocations(self, ctx: RenderContext) -> list:
        ...
        allocations = []
        visible_children = [c for c in self._children 
                           if c.get_render_state() != RenderState.HIDDEN]
