...
class InteractiveCommandFilter:
    ...
    def _has_interactive_attributes(cls, instance) -> bool:
        ...
        for attr in dir(instance):
            ...
            if (
                attr_lower in cls.UI_ATTRIBUTE_PATTERNS
                or attr_lower.startswith("interactive")
                or attr_lower.endswith("_ui")
                or attr_lower.startswith("ui_")
            ):
                return True
        return False
    ...
    def get_tool_schema(self, cmd_name: str, cmd_class: type) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            tool_schema = {
                ...
                "description": getattr(
                    instance, "description", f"Execute hyper {cmd_name} command"
                ),
        ...
        try:
            ...
            for param_name, param in sig.parameters.items():
                ...
                if param.annotation is bool or (
                    param.default != inspect.Parameter.empty and isinstance(param.default, bool)
                ):
...
class MCPCommandExecutor:
    ...
    def execute_command(
        self, cmd_name: str, cmd_class: type, arguments: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for param_name, param in sig.parameters.items():
            if param_name == "args" and param.kind == inspect.Parameter.VAR_POSITIONAL:
                ...
                if "args" in arguments:
                    extra_args = (
                        arguments["args"]
                        if isinstance(arguments["args"], list)
                        else [arguments["args"]]
                    )
    ...
    def _build_response(
        ...
        if exit_code == 0:
            ...
            content.append(
                {"type": "text", "text": f"Command '{cmd_name}' failed with exit code {exit_code}"}
            )
...
class MCPServer:
    ...
    def get_command_info(self) -> dict[str, Any]:
        ...
        for cmd_name in self.registry.list_commands():
            ...
            if self.command_filter.is_interactive(cmd_name, cmd_class, self.container):
                reason = self.command_filter.get_interactive_reason(
                    cmd_name, cmd_class, self.container
                )
        ...
        if self.command_filter.is_interactive(cmd_name, cmd_class, self.container):
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Command '{cmd_name}' is interactive and cannot be executed via MCP",
                    }
                ],
                "isError": True,
            }
    ...
    def _handle_interactive_resource(
        self, uri: str, command_info: dict[str, Any]
    ) -> dict[str, Any]:
